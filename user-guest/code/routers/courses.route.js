...
Router.get("/search", async (req, res) => {
  ...
  if (idCourseTopic == undefined) {
    var idCourseTpc = await CourseTopic.find({name:queryString.trim()})

    if(idCourseTpc != null && idCourseTpc != undefined && idCourseTpc.length > 0  )
    {
      // console.log("id = " + idCourseTpc);
      var tempCourseTpc = JSON.stringify(idCourseTpc).substring(1, JSON.stringify(idCourseTpc).length - 1);
      tempCourseTpc = JSON.parse(tempCourseTpc);
      // console.log("tempCourseTpc = " + tempCourseTpc["_id"]);
      courses = await Course.find({ idCourseTopic: tempCourseTpc["_id"] });
